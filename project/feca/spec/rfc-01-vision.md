...
Le projet consiste à développer une plateforme permettant d'automatiser la production de contenus vidéo courts à partir de sujets d'actualité.

La plateforme repose sur une architecture **multi-agents IA**. Chaque agent possède une responsabilité précise et communique avec les autres agents au travers d'un workflow orchestré.
...
L'objectif n'est pas de créer un agent unique capable de tout faire, mais de construire une chaîne de production composée de services spécialisés, observables et remplaçables indépendamment.
...
```text
**Recherche web → Analyse → Rédaction → Vérification → Voix → Avatar → Montage → Contrôle qualité → Publication**
